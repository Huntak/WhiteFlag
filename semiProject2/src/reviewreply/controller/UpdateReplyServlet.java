package reviewreply.controller;

import java.io.IOException;

import javax.servlet.RequestDispatcher;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import reviewreply.model.service.ReviewReplyService;
import reviewreply.model.vo.ReviewReply;

/**
 * Servlet implementation class UpdateReplyServlet
 */
@WebServlet("/updatere")
public class UpdateReplyServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public UpdateReplyServlet() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		response.setContentType("text/html; charset=utf-8");
		int rrnumber = Integer.parseInt(request.getParameter("rrnumber"));
		String rrcontent = request.getParameter("rrcontent");
		if(rrcontent != null && !rrcontent.equals("")){
			rrcontent = new String(rrcontent.getBytes("8859_1"), "utf-8");
		}
		int rnumber = Integer.parseInt(request.getParameter("rnumber"));
		int result = new ReviewReplyService().updateReply(rrnumber, rrcontent);
		RequestDispatcher view = request.getRequestDispatcher("rdetail");
		request.setAttribute("rnumber", rnumber);
		view.forward(request, response);
	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		doGet(request, response);
	}

}
