...
/**
 * Servlet implementation class EditPlusInput
 */
@WebServlet("/editPlus")
public class EditPlusInput extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public EditPlusInput() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//PID, PLUSPRICE, PLUSQUANTITY, PLUSTOTAL 필요.	 
		 String plus = request.getParameter("plusEditArray");
		...
		// System.out.println(plusArray[1].substring(1));
		 for(int i=0; i<plusArray.length; i++){  
			 ...
			 if(plusArray.length==1 && i==0) {plusArray[i] =plusArray[i].substring(2,plusArray[i].length()-2) ;}
			 else {
			 ...
			 // 입고량이 0인 항목을 제외한다.
			 if(!eachPlusArray[3].equals("0")){ 
				 Plus pl = new Plus(eachPlusArray[0],eachPlusArray[1],eachPlusArray[2],eachPlusArray[3],eachPlusArray[4] );
				 plusArrayForPlus.add(pl);
				 //countPlusObject ++; 
				 
				 System.out.println(eachPlusArray);
			}
		 }
		 ...
		 for(int j=0; j<plusArrayForPlus.size(); j++ ){
	      ...
	      int result = new PlusService().updatePlus(plusArrayForPlus);
