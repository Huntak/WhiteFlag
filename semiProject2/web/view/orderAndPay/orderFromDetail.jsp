<%@ page language="java" contentType="text/html; charset=UTF-8"
 ...
 <%
		...
		ProductImage pimage = (ProductImage)request.getAttribute("pimage");
 		int quantity = (int)request.getAttribute("quantity");
 		int mileage = (int)request.getAttribute("mileage");
...
$("#insert").click(function(){
	...
	alert('마일지가 부족합니다');
  		...
  		var total = <%= Product.getpPrice() *quantity + 2500%>
		    ...
		    if ( rsp.success ) {
		       ...
		       var minus2 = $('#point').val();
		        ...
		        var mid = $('#loginuser').val();
		     	...
		     	var osnumber = '2';
		     	var omiliage = <%= mileage %>;
		     	var oquantity = <%= quantity %>
		     	var total2 = <%= Product.getpPrice() * quantity + 2500%> - minus2
		     	var varray = [];
		      ...
		      //ajax 로 orderlist 값 보내기
		     	...
		     	$.ajax({
		     		url : "/semi/payment",
		     		data : {mid : mid, pid : pid, osnumber : osnumber, omiliage : omiliage, oquantity : oquantity, total2 : total2, varray : varray},
		     		type : "post"	
		     	});	//ajax   
		 		...
		 		//결제 완료 페이지
		 		location.href="/semi/success?pname=<%= Product.getpName()%>&psize=<%= Product.getpSize()%>&pcolor=<%= Product.getpColor()%>"+
		 		"&quantity=<%= quantity %>&mid=<%= loginUser.getMID()%>&mainimage=<%= pimage.getMainImage()%>&imageid=<%= pimage.getImageId()%>";
...
<table id="product_table">
  		...
  		<td width="12%;"><a href="/semi/detail?imageId=<%= pimage.getImageId() %>" ><image src="/semi/image/<%= pimage.getMainImage() %>"  width="120" height="170	"></a></td>
  		<td width="40%;"><a href="/semi/detail?imageId=<%= pimage.getImageId()  %>" ><%= Product.getpName() %></a></td>
  		<td width="7%;"><%= Product.getpPrice() %></td>
  		<td width="2%;"><%= quantity %></td>
  		<td width="4%;"><%= mileage  %>점</td>
  		<td width="4%;">조건</td>
  		<td width="7%;"><%= Product.getpPrice() * quantity %></td>
  		</tr>
...
<tr id="pay1_tr2"><td><%= Product.getpPrice() * quantity%>원</td>
<td class="paytable1_td"><input type="text" id="dc_td" value="0점" readonly></td>
<td class="paytable1_td"><input type="text" id="totalpay" value="<%= Product.getpPrice() * quantity + 2500%>원" readonly></td></tr>
...
-적립급은 최소 10,000원 부터 사용 가능합니다.<br>
...
<div><input type="text"  id="final_pay_input" value="<%= Product.getpPrice() * quantity + 2500%>원" readonly></div><p>
